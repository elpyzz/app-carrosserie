# Plan : Recréation complète de la page Paramètres (CORRIGÉ ET COMPLET)
...
- **Style** : TailwindCSS avec classes `bordeaux-*` pour les couleurs principales
...
✅ **Checkbox avec Controller** - Utiliser `Controller` de react-hook-form pour les Checkbox (même si c'est un input HTML, Controller garantit la synchronisation)  
    ...
    <AuthenticatedLayout>
      <div className="space-y-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          {/* Voir sections JSX complètes ci-dessous */}
        </form>
      </div>
    </AuthenticatedLayout>
  )
...
Le composant Checkbox de shadcn/ui utilise `onCheckedChange` au lieu de `onChange`, donc `register()` ne fonctionne pas.

**Solution :** Utiliser `Controller` de react-hook-form :
...
- Le style utilise les classes `page-title` et `btn-primary` pour la cohérence avec le reste de l'application
- Les Checkbox utilisent `onCheckedChange` (propriété shadcn/ui) et non `onChange` (input HTML standard)
